...
export class DashboardComponent implements OnInit {
   ...
   */
    ...
    public barChartLabels: Label[] = [ "4/12", "4/12","4/12","4/12","4/12", "4/12", "4/12", "4/12", "4/12"];
    public barChartType: ChartType = 'bar';
    public barChartColors: Color[] = [
      { backgroundColor: ["#7b65c9","#39e64d","#7b65c9","#39e64d","#7b65c9", "#39e64d", "#7b65c9", "#39e64d", "#7b65c9"] }
    ]
    public barChartLegend = false;
    public barChartPlugins = [];
    public barChartData: ChartDataSets[] = [
      { data: [24,52,44,20,14,20,32,36,17], label: 'Population' }
    ];
   ...
   */
     ...
     public doughnutChartData: SingleDataSet = [50,20];
     public doughnutChartType: ChartType = 'doughnut';
     public doughnutChartLabels: Label[] = ["Present", "Absent"];
     public doughnutChartColors: Color[] = [
       { backgroundColor: ["#39e64d","#7b65c9"] }
     ];
   ...
   */
     ...
     public smalldoughnutChartData: SingleDataSet = [72,10,10];
     public smalldoughnutChartType: ChartType = 'doughnut';
     public smalldoughnutChartLabels: Label[] = ["Done", "Pending"];
     public smalldoughnutChartColors: Color[] = [
       { backgroundColor: ["#7b65c9","#3f3d47"] }
     ];
   



  constructor(private calendar: NgbCalendar, private userService : userService , private datepipe: DatePipe , private attendanceService : attendanceService , private modalService: NgbModal) {
  ...
  roleE = "6257bb97a3cc8e08e46399fd";
  users = [];
  myResearcherUsers = [];
  myEmployeeCount = 0 ;
  usersE = [];
    ...
    this.userService.getUsers(conditions).subscribe((response) => {
      ...
      for(let i=0, j=0; i<this.users.length; i++){
        if(this.users[i]['RoleId'] == this.role){
          ...
          this.myEmployeeCount = this.myResearcherUsers.length;
  ...
  fetchEUser() {
    ...
    this.userService.getUsers(conditions).subscribe((response) => {
      console.log("pehly")
      console.log(response);
      this.usersE = response;
      for(let i=0, j=0; i<this.usersE.length; i++){
        if(this.usersE[i]['RoleId'] == this.roleE){
          this.myResearcherUsersE[j] = this.usersE[i];
          console.log("My after this");
          this.myEmployeeCountE = this.myResearcherUsersE.length;
         console.log(this.myResearcherUsersE);
          j++;
          console.log("inside If Statement ");
        }
      }
  ...
  fetchAttendance(){
    ...
    this.attendanceService.getAttendance(conditions).subscribe((response) =>{
      ...
      for(let i=0 , j=0 , k =0 , l=0 , m=0 , n=0 , o=0 , p=0 , q=0; i < this.attendance.length ; i++){
        if(true){
        this.attendanceverification[j] = this.attendance[i];
        this.countRecivedAttendance = this.attendanceverification.length;
        if(this.attendance[i]['location'] == 'out' || this.attendance[i]['currentDate'] == this.currentDateTime){
          this.one[k] =this.attendance[i];
          this.countOne = this.one.length; 
          k++;
        }
        if(this.attendance[i]['location'] == 'in' || this.attendance[i]['currentDate'] == this.currentDateTime){
          this.two[l] =this.attendance[i];
          this.countTwo = this.two.length;
          l++;
        }
        if(this.attendance[i]['status'] == 'Absent' || this.attendance[i]['currentDate'] == this.currentDateTime){
          this.three[m] =this.attendance[i];
          this.countThree = this.three.length;
          m++;
        }
        if(this.attendance[i]['status'] == 'Present' || this.attendance[i]['currentDate'] == this.currentDateTime){
          this.four[n] =this.attendance[i];
          this.countFour = this.four.length;
          n++;
        }
        if(this.attendance[i]['status'] == 'Leave' || this.attendance[i]['currentDate'] == this.currentDateTime){
          this.five[o] =this.attendance[i];
          this.countFive = this.five.length;
          o++;
        }
        if(this.attendance[i]['entery'] == 'pin' || this.attendance[i]['currentDate'] == this.currentDateTime){
          this.six[p] =this.attendance[i];
          this.countSix = this.six.length;
          p++;
        }
        if(this.attendance[i]['entery'] == 'thumb' || this.attendance[i]['currentDate'] == this.currentDateTime){
          this.seven[q] =this.attendance[i];
          this.countSeven = this.seven.length;
          q++;
        }
        j++;
        }
